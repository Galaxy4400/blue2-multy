function mainFormAfterSubmit({ success, ...payload }) {
	success ?
		mainFormSuccessHandler(this, payload) :
		mainFormErrorHandler(this, payload);
}
...
function mainFormErrorHandler(form, { code, errors }) {
	code === 'invalid_params' ?
		renderInvalidParamsError(form, errors) :
		renderRegistrationError(form);
}
...
function helpFormAfterSubmit({ success }) {
	success ?
		renderHelpFormSuccess(this) :
		renderHelpFormError(this);
}
