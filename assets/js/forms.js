...
document.querySelectorAll('[placeholder]').forEach(input => {
	input.placeholder = local(input.placeholder);
});
...
document.querySelectorAll('input[data-radio]').forEach(radio => {
	...
	if (radio.type != 'radio') { console.error("The element should be input[type='radio']"); return; }
	...
	if (isCustom && (radioLabel.tagName.toLowerCase() != 'span' || radioLabel.dataset.radioCustom == undefined)) {
		console.error('The capture field of the radio video should be Span and have an attribute data-radio-custom');
		return;
	}
...
document.querySelectorAll('input[data-check]').forEach(check => {
	...
	if (check.type != 'checkbox') { console.error("The element should be input[type='checkbox']"); return; }

	check.dataset.label = local(check.dataset.label);
...
document.querySelectorAll('input[data-file]').forEach(input => {
	...
	if (input.type != 'file') { console.error("The element should be input[type='file']"); return; }
...
function initCountryPhones(form) {
	...
	fetchUserCountry()
		.then(country => form.iti.setCountry(country))
		.finally(() => formUnloading(form));
}
...
function validateCountryPhone(form) {
	...
	if (isValid) {
		...
		errorBlock.innerHTML = `<div class="form__error-label just-validate-error-label" style="color: rgb(184, 17, 17);">${local('Wrong format')}</div>`;
	}
				...
				validator: (value) => () =>
					new Promise((resolve) => {
						...
						fetch(action)
							...
							.then(isExist => resolve(isExist.toLowerCase() === 'true')) // (true/false)
...
function clearForm(form) {
	form.querySelectorAll('input[type="text"], input[type="email"], input[type="tel"], input[type="password"], textarea').forEach(item => {
